...
export const SubmitButton = ({ label }: { label: string }) => {
  ...
  return (
    <button
      ...
      disabled={pending}
    >
      {label === "upload" ? <>{pending ? "uploading..." : "upload"}</> : <>{pending ? "uploading..." : "update"}</>}
    </button>
  );
};
...
export const EditButton = ({ id }: { id: string }) => {
  return (
    <Link href={`/edit/${id}`} className='py-3 text-sm bg-gray-50 rounded-bl-md w-full hover:bg-gray-100 text-center'>
      Edit
    </Link>
  );
};
